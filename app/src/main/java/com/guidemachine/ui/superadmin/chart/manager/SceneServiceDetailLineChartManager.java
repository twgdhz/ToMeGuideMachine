package com.guidemachine.ui.superadmin.chart.manager;
...
import com.guidemachine.ui.superadmin.chart.view.LineChartMarkView;
 ...
 * @create 2018/12/14 0014 10:20
 * description: 折线图管理类
 */
public class SceneServiceDetailLineChartManager {
    ...
    public SceneServiceDetailLineChartManager(LineChart lineChart) {
...
//        description.setText("需要展示的内容");
        ...
        leftYAxis.setDrawAxisLine(true);//是否绘制轴线
        leftYAxis.setDrawGridLines(false);//设置y轴上每个点对应的线
        //设置Y轴网格线为无
        leftYAxis.enableGridDashedLine(0, 1f, 0f);
    ...
    public void setXAxisData(float min, float max, int labelCount) {
        ...
        xAxis.setLabelCount(labelCount, false);////设置X轴的显示个数
     ...
     * @param color    曲线颜色
     */
    public void showLineChart(final List<SceneryServiceInfoDetail.DataBean> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            SceneryServiceInfoDetail.DataBean data = dataList.get(i);
            /**
             * 在此可查看 Entry构造方法，可发现 可传入数值 Entry(float x, float y)
             * 也可传入Drawable， Entry(float x, float y, Drawable icon) 可在XY轴交点 设置Drawable图像展示
             */
            Entry entry = new Entry(i, data.getOfflineNum());
            entries.add(entry);
        }
...
//                return DateUtil.formatDateToMD(tradeDate);
        ...
        leftYAxis.setLabelCount(6);//设置y轴的显示个数
        ...
        leftYAxis.setAxisLineWidth(1f);//设置x轴线宽度
     ...
     * 添加曲线
     */
    public void addLine(List<SceneryServiceInfoDetail.DataBean> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            SceneryServiceInfoDetail.DataBean data = dataList.get(i);
            Entry entry = new Entry(i, (int) data.getOnlineNum());
            entries.add(entry);
        }
        // 每一个LineDataSet代表一条线
        LineDataSet lineDataSet = new LineDataSet(entries, name);
    ...
    public void addTotal(List<SceneryServiceInfoDetail.DataBean> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            SceneryServiceInfoDetail.DataBean data = dataList.get(i);
            Entry entry = new Entry(i, (int) data.getTotalNum());
            entries.add(entry);
        }
        // 每一个LineDataSet代表一条线
        LineDataSet lineDataSet = new LineDataSet(entries, name);
    ...
    public void addTotalDeviceLine(List<AdminStatisticsBean> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            AdminStatisticsBean data = dataList.get(i);
            Entry entry = new Entry(i, (int) data.getTotalNum());
            entries.add(entry);
        }
        // 每一个LineDataSet代表一条线
