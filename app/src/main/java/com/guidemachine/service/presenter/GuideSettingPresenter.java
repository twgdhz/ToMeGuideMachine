...
public class GuideSettingPresenter implements Presenter  {
    private DataManager manager;
    private CompositeSubscription mCompositeSubscription;//用来存放RxJava中的订阅关系
    private Context mContext;
    private BaseView mBaseView;
    private BaseBean mBaseBean;
    public GuideSettingPresenter(Context mContext) {
    ...
    public void releaseJourney(RequestBody requestBody) {//网络请求就开始
        mCompositeSubscription.add(manager.shutDown(requestBody)//向CompositeSubscription添加一个订阅关系
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<BaseBean>() {
                    @Override
                    public void onCompleted() {
                        L.gi().d("获取返回结果:"+mBaseBean.toString());
                        //0000请求成功码
