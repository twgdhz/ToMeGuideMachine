...
import com.guidemachine.base.adapter.Listener.OnDragAndSwipeListener;
import com.guidemachine.base.adapter.Listener.OnRecyclerItemClickListener;
import com.guidemachine.base.adapter.Listener.OnRecyclerItemLongClickListener;
...
//    private View mLoadView;
   ...
   private static final int VIEW_TYPE_HEADER = 1;//header
    private static final int VIEW_TYPE_CONTENT = 2;//content
    private static final int VIEW_TYPE_FOOTER = 3;//footer
    private static final int VIEW_TYPE_EMPTY = 4;//empty
    //private static final int VIEW_TYPE_LOADING = 5;//loading
...
//        int count = mData.size() + getHeaderViewCount() + getFooterViewCount() + loadMoreCount;
		int count = mData.size() + getHeaderViewCount() + getFooterViewCount();
...
//        Logger.d("调用一次onBindViewHolder");
//        Logger.d(mViewType);
//        Logger.d(holder.getLayoutPosition());
        switch (mViewType) {
            case VIEW_TYPE_HEADER:
                break;
            case VIEW_TYPE_EMPTY:
            ...
            default:
            case VIEW_TYPE_CONTENT:
                convert((BaseViewHolder) holder, mData.get(holder.getLayoutPosition() - getHeaderViewCount()),position);
                break;
//            case VIEW_TYPE_LOADING:
//                addLoadMore();
//                break;
    ...
    public  boolean isLoadMore() {
        return mNextLoadingEnable&& mData.size() > 0;
    }
...
//                            || type == VIEW_TYPE_LOADING) ? gridManager.getSpanCount() : 1;
                    return (type == VIEW_TYPE_EMPTY || type == VIEW_TYPE_HEADER || type == VIEW_TYPE_FOOTER) ? gridManager.getSpanCount() : 1;
                }
            });
        }
    }
...
////            mFooterView.setBackground(mContext.getResources().getDrawable(R.mipmap.ic_toolbar));
//			this.notifyDataSetChanged();
//		}
//    }
//
//    public void addClickMoreView(View.OnClickListener listener) {
//        mNextLoadingEnable = false;
//        if(mFooterView == null) {
//            mFooterView = new FooterView(mContext);
//            ((FooterView) mFooterView).setClickTomore();
